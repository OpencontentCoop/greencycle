...
<div class="editorialstuff u-margin-bottom-xl">
    ...
    <table class="table table-striped" cellpadding="0" cellspacing="0" border="0">
      <tr>
        <th><small></small></th>
        <th><small>{'Municipality'|i18n('editorialstuff/dashboard')}</small></th>
    ...
    {foreach $posts as $post}
      <tr>
          ...
          <td>
            {def $section = fetch( 'section', 'object', hash( 'section_id', $post.object.section_id ))}
            {if $section.identifier|begins_with('municipality_')}
              {$section.name|wash()}
            {/if}
            {undef $section}
          </td>
